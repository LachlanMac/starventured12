...
const TraitSelection: React.FC<TraitSelectionProps> = ({
  ...
  const filteredPositiveTraits = positiveTraits.filter(trait => 
  ...
  const filteredNegativeTraits = negativeTraits.filter(trait => 
    ...
    <div>
      ...
      <div style={{ 
        ...
        <p>
          ...
          <strong style={{ color: 'var(--color-metal-gold)' }}> Positive traits</strong> cost 1 module point each, while 
        ...
        <CardBody>
          ...
          <div style={{ 
            ...
            {filteredPositiveTraits.map(trait => {
              ...
              const canSelect = !isSelected && selectedTraits.length < 3 && availableModulePoints >= 1;
        ...
        <CardBody>
                ...
                >
                  <div style={{ 
                    ...
                    <Button 
                      variant="outline" 
                      size="sm"
                      onClick={() => onDeselectTrait(trait._id)}
                    >
                      Remove
                    </Button>
                  </div>
          ...
          {selectedTraits.length < 3 && (
            <div style={{ 
              ...
              Please select {3 - selectedTraits.length} more trait{selectedTraits.length === 2 ? '' : 's'} to continue.
            </div>
          )}
        </CardBody>
      </Card>
...
const getMockPositiveTraits = (): Trait[] => [
  {
    ...
    name: "Tech Savvy",
    type: "positive",
    description: "You have a natural affinity for technology. Gain advantage on all Technology checks and can reroll one failed Technology check per game session."
  },
  {
    _id: "pos2",
    name: "Fearless",
    type: "positive",
    description: "You are immune to fear effects and gain +1 to all Presence checks when attempting to intimidate or lead others in dangerous situations."
  },
  {
    _id: "pos3",
    name: "Quick Reflexes",
    type: "positive",
    description: "You have exceptionally fast reflexes. You gain +2 to Initiative and can reroll one Evade check per combat encounter."
  },
  {
    _id: "pos4",
    name: "Natural Leader",
    type: "positive",
    description: "Your natural charisma inspires others. Allies within 10 units gain +1 to their next roll after you succeed on a check with a roll of 18 or higher."
  },
  {
    _id: "pos5",
    name: "Intuitive",
    type: "positive",
    description: "You have a natural intuition about people and situations. Once per game session, you can ask the GM one yes/no question that your character would have no way of knowing."
  }
];
...
const getMockNegativeTraits = (): Trait[] => [
  {
    ...
    type: "negative",
    description: "You have social anxiety disorder. If you roll a 1 on any dice during a social check, you lose 1 willpower. Additionally, until your next full rest, you always use the lowest dice roll on your social checks instead of the highest when rolling with talent bonuses."
  },
  {
    _id: "neg2",
    name: "Phobia: Darkness",
    type: "negative",
    description: "You have an intense fear of darkness. While in dark areas, you suffer disadvantage on all checks and must make a Resilience check (DC 15) each round or be unable to take actions other than moving toward a light source."
  },
  {
    _id: "neg3",
    name: "Addiction",
    type: "negative",
    description: "You are addicted to a substance. Every 24 hours without it, you suffer cumulative -1 penalties to all checks until you take a dose or complete a full detox (3 days of penalties)."
  },
  {
    _id: "neg4",
    name: "Bad Luck",
    type: "negative",
    description: "You are plagued by bad luck. Once per game session, the GM can force you to reroll a successful check and take the new result."
  },
  {
    _id: "neg5",
    name: "Chronic Injury",
    type: "negative",
    description: "You have a chronic injury that occasionally flares up. At the start of each day, roll 1d20. On a 1-5, the injury flares up, imposing disadvantage on physical checks until you take a short rest."
  }
];
